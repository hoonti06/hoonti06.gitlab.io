...
// 참고
// ([^]*) 은 개행까지 포함한다. ([\w\W]*)로 표현할 수도 있다.
// (.*)은 개행을 포함하지 못한다.


function replaceCodeBlockWithRougeLiquid(fileContent)
{
  const regex = /\h*\`\`\`(.*)\n([^]*?)\h*\`\`\`\h*(\n|$)/g;
...
//  console.log("filePath : " + filePath);
  ...
  var result = replaceCodeBlockWithRougeLiquid(data);
