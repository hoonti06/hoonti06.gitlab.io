...
updated   : 2021-06-01 08:57:14 +0900
tag       : 
public    : true
published : true
parent    : 
...
{:toc}
## 2021-06-01
- gitlab의 물 주기와 github의 잔디 심기가 따로 되어 있어 gitlab page를 github에서 mirroring하려고 한다
 
## 2021-05-31
- [lint 테스트 없이 push]{https://github.com/TEAMMATES/teammates/pull/11151/commits/6611d6d04ab1c8281f881155582700ce47bdc257 } 했더니 teamates lint 테스트에서 실패했다.  
	- `./gradlew lint`로 테스트할 수 있는데, if문에 중괄호를 생략(`[ant:checkstyle] [ERROR] /Users/hoon/workspace/teammates/src/e2e/java/teammates/e2e/pageobjects/InstructorAuditLogsPage.java:63:21: 'if' construct must use '{}'s. [NeedBraces]`)했다고 실패했다
	- [이 commit]{https://github.com/TEAMMATES/teammates/pull/11151/commits/6611d6d04ab1c8281f881155582700ce47bdc257 }에서 중괄호를 추가했더니 `/Users/hoon/workspace/teammates/src/e2e/java/teammates/e2e/pageobjects/InstructorAuditLogsPage.java:61: Avoid declaring a variable if it is unreferenced before a possible exit point.`라고 또 실패했다.
		- 위 에러는 변수 sessionName이 정의되고 return 문(exit point) 위에서 한 번도 참조되지 않아서 발생한 에러이다.
		- lint에만 문제가 있는 줄 알았는데, e2e test도 또 실패했다.
			- WebElement의 findElement method는 첫 번째 매칭되는 element를 리턴하는데 하나도 없으면 
NoSuchElementException이 발생한다. 그래서 해당 exception이 발생한 것이었다. 호출하는 method를 잘 살펴봤어야 했는데, 그러지 못해서 발생한 에러이다. 아무튼, 처음에는 try-catch 문을 통해 해당 exception을 처리할까 했는데, 찾아보니 [stackoverflow]{https://stackoverflow.com/a/6521834 } 에서 findElements() method를 호출하고 그 결과가 empty인지 확인하면 된다고 한다.

```java
if (){ // ')' 와 '{'에 띄어쓰기 안 하면 다음과 같은 에러도 발생한다
...
- teammates test에서 무조건 down(메뉴 펴기, expand) 버튼을 클릭하도록 되어 있는데, 내가 커밋한 것은 log가 없으면 이미 펴져 있으므로, down(expand) 버튼은 없고 up(collapse, 접기) 버튼 밖에 없어 [NoSuchElementException]{https://github.com/TEAMMATES/teammates/pull/11151/checks?check_run_id=2700997176#step:10:100 } 이 발생했던 것이다.
...
- Java open source project인 teammates에 한 이슈에 대한 수정 사항을 [pull request]{https://github.com/TEAMMATES/teammates/pull/11151 } 요청했다
	- beginner가 하기 좋은 Java open source project 리스트 중의 하나인 project라서 둘러보다가 이슈가 간단해서 하게 되었다. 근데, UI 부분이라서 Java는 아니고 Angular와 TypeScript로 되어 있었다. 간단히 array의 길이가 0일 때, tab이 expand될 수 있도록 하였다.
	- spec의 하나로 contribute를 하게는 되었는데, 이게 나에게 맞는 spec일까 싶기도 하다
...
## 2021-05-21
- mybatis mapper.xml을 resources에 두지 않고, src(java package)에 두면 intellij의 경우 mapper.xml을 갱신하더라도 실행할 때 실행 폴더(maven의 `target/classes`, gradle의 경우 `out/production/classes`)로 복사하지 않는다. (eclipse는 가리지 않고 실행 폴더에 잘 넣어준다)
	- maven project의 경우 pom.xml에 다음과 같이 설정한다.   
		```xml
		<build>
			...
			<resources>
		...
		```
		- 참고 : <https://okky.kr/article/584249>

	- gradle project의 경우 gradle.build에 다음과 같이 설정한다.(검증 필요)
		```groovy
		sourceSets {
			main {
		...
		```
		- 참고 : <https://discuss.gradle.org/t/how-to-add-xml-files-from-java-source-directory-to-output-jar/4519>
