...
updated   : 2022-01-27 23:03:30 +0900
jag       : 
public    : true
published : true
parent    : [[memo]]
...
## 2021-09-30
- AES128, 192, 256
	- 보안에 128이면 충분하다 bit가 커질 수록 당연히 보안성이 더 높아지겠지만, CPU overhead도 커진다.
	- 참고 : <https://crypto.stackexchange.com/a/23>
- 포트폴리오 작성 방법
...
## 2021-09-27
- github actions workflow의 job 사이에 빌드된 도커 이미지 공유 방법 : [링크](https://github.com/docker/build-push-action/blob/master/docs/advanced/share-image-jobs.md )

## 2021-09-26
- [[openjdk]]
 
## 2021-09-25
- `docker system prune -f`로 다음 항목을 제거한다
...
## 2021-09-09
- vue에서 환경 변수 사용 방법(참고 : <https://stackoverflow.com/a/55628504>)
	- `.env.production`의 env variable에 `VUE_APP_`를 prefix로 붙여야 한다(e.g. VUE_APP_WHATEVERYOUWANT)
	- `process.env.VUE_APP_WHATEVERYOUWANT`로 사용하면 된다.
- 도움이 될만한 웹 페이지
	- redis pub/sub 모델 : <https://daddyprogrammer.org/post/3688/redis-spring-data-redis-publish-subscribe/>
	- redis expiration event 리스너 : <https://stackoverflow.com/questions/41506606/spring-data-redis-listen-to-expiration-event>
	- certbot docker를 이용한 SSL 인증서 발급 받기 : <https://velog.io/@hytenic/SSL-Lets-Encrypt-Certbot-docker%EB%A5%BC-%EC%9D%B4%EC%9A%A9%ED%95%98%EC%97%AC-SSL-%EC%9D%B8%EC%A6%9D%EC%84%9C-%EB%B0%9C%EA%B8%89%EB%B0%9B%EA%B8%B0>
	- 인터넷의 근본에 대한 이야기 : <https://bit.ly/2YKMqJ1>
	- 이더리움
		- docker 위에 geth 설치 : <http://melonicedlatte.com/blockchain/2018/03/25/202045.html>
...
## 2021-09-05
- 도움이 될만한 웹 페이지
	- healthcheck 및 docker swarm : <https://subicura.com/2017/02/25/container-orchestration-with-docker-swarm.html>
		- keywords : healthcheck, overlay network 
	- docker를 통한 무중단 배포 : <https://subicura.com/2016/06/07/zero-downtime-docker-deployment.html>
		- keywords : docker-gen, blue-green deploy
	- docker-compose와 docker stack의 차이 : <https://log-laboratory.tistory.com/191>
		- keywords : 서로 무시하는 yml 옵션
	- swarm, overlay network : <https://javacan.tistory.com/entry/docker-start-11-swarm-network>
		- keywords : swarm, overlay network, yml 'deploy' 옵션
	- stack deploy reference : <https://docs.docker.com/engine/swarm/stack-deploy/>
	- docker ochestration 종류 : <https://archived.realignist.me/code/2017/06/14/docker-my-usecase.html>
		- keywords : watchtower 
	- watchtower : <https://blog.banyazavi.com/2021-01-30/Watchtower-%EB%8F%84%EC%BB%A4-%EC%9D%B4%EB%AF%B8%EC%A7%80-%EC%97%85%EB%8D%B0%EC%9D%B4%ED%8A%B8-%EC%9E%90%EB%8F%99%ED%99%94>
	- <https://stackoverflow.com/questions/26423515/how-to-automatically-update-your-docker-containers-if-base-images-are-updated>
- compose vs stack
	- docker-compose는 container를 정의하는 것이고, docker stack는 service를 정의하는 것
	- stack은 monitoring과 에 따른 조치 가능
	- stack은 swarm에서만 사용 가능
	- compose는 따로 설치해야 한다
- docker swarm
	...
	- docker-composel.yml으로 docker stack을 실행할 때엔 `build`, `privileged`, `restart` 옵션은 무시된다

		
 
## 2021-09-02
- [[openvidu-e2e-test]]
- MySQL을 쓴다면 MariaDB로 migration해야 하는 이유 : <https://linuxnatives.net/2015/10-reasons-to-migrate-to-mariadb-if-still-using-mysql>
 
- [[blockchain]]
 
## 2021-08-31
- jenkins의 plugin blue ocean 사용 방법 : <https://medium.com/@jyson88/jenkins-blue-ocean-%EC%82%AC%EC%9A%A9-%EB%B0%A9%EB%B2%95-579d8059a7b7>
- portainer으로 private registry 구성하는 방법 : <https://www.youtube.com/watch?v=ZK-3H8_bHKY>
 
## 2021-08-30
- CI에 sonatype/nexus3를 docker container로 설치해서 실행했는데, 메모리가 충분하지 않다고 에러가 발생했다. [reference](https://help.sonatype.com/repomanager3/installation/system-requirements#SystemRequirements-GeneralMemoryGuidelines )를 찾아보니 최소 8GB 이상이어야 하는 듯 하다..
...
## 2021-08-27
- Cloud provider 대부분이 container 이미지 remote 저장소 서비스를 제공하고 있다.
	- Github의 [GitHub Container Registry](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry )
		- 500MB까지 무료 
		- [가격 참고](https://github.com/features/packages#pricing )
	- AWS의 [ECR(Elastic Container Registry)](https://aws.amazon.com/ko/ecr/ )
		- AWS 프리 티어의 일부로서 1년 동안 월 500MB의 Private 저장소 제공
		- Public 저장소는 50GB까지 무료 
		- Outbound 트래픽에 대한 비용이 500GB(익명)까지 무료, AWS Account으로 Pull한 경우 5TB까지 트래픽 사용료 무료
		- [가격 참고](https://aws.amazon.com/ko/ecr/pricing/?loc=ft#AWS_Free_Tier )
	- Google의 [GCR(Google Container Registry)](https://cloud.google.com/container-registry )
		- 멀티 리전의 표준 버킷 가격은 월별 GB당 약 $0.026 
		- [가격 참고](https://cloud.google.com/container-registry/pricing?hl=ko )
