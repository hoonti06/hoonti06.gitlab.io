...
# github에 올린 user-images를 자동으로 다운로드합니다. (created by johngrib)
...
for CHANGED_FILE in $CHANGE_LIST; do
    ...
    mkdir -p ./wiki-img/$DIR_NAME
    URI_LIST=`ag "https://user-images\.githubuser.*?\/$NUM\/.*?(png|jpg|gif)" -o $CHANGED_FILE`

    for URI in $URI_LIST; do
        FILE_NAME=`echo $URI | sed 's,^.*/,,'`
        echo "TARGET: $URI"
        curl -s $URI > ./wiki-img/$DIR_NAME/$FILE_NAME

        if [ "$?" == "0" ]; then
            echo "DOWNLOAD SUCCESS: $FILE_NAME"
            sed -i '' -E 's, *https://.*('"$FILE_NAME"') *, /wiki-img/'$DIR_NAME'/\1 ,g' $CHANGED_FILE

            git add wiki-img/$DIR_NAME/$FILE_NAME

            SUCCESS_COUNT=$((SUCCESS_COUNT+1))
        else
            echo "DOWNLOAD FAIL: $FILE_NAME"
            rm -f ./wiki-img/$DIR_NAME/$FILE_NAME
